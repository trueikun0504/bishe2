...
6. `conversations`：`buyer_openid + updated_at`、`seller_openid + updated_at`
7. `messages`：`conversation_id + created_at`
8. `feedback`：`created_at`
9. `orders`：`order_id + status`
...
收藏功能由 `/api/web/favorites/toggle` 驱动，前端在详情页或收藏页中都可以进行收藏状态切换。系统会将收藏 ID 同步写入用户文档和 `favorites` 集合，便于兼容多种数据读取路径。
...
当用户在商品详情页点击“联系卖家”时，前端调用 `/api/web/conversations/open`。若当前商品与买卖双方已存在会话，则复用已有会话；否则创建新会话。进入详情页后，系统会拉取 `/api/web/conversations/:id/messages` 并在发送消息时调用 `/api/web/conversations/:id/messages` 完成消息写入。

消息发送成功后，服务端会同步更新 `conversations` 集合中的 `last_message`、`unread_count` 和 `updated_at` 字段，并通过 WebSocket 向会话参与者广播 `message:new` 事件。
